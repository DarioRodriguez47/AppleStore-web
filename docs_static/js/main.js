...
(function(){
  ...
  function renderList(){
    ...
    productos.forEach(p=>{
      ...
      html.push(`<div class="product-info"><span class="product-name">${escapeHtml(p.nombre)}</span><span class="product-description">${escapeHtml(p.descripcion)}</span></div>`);
      html.push('</li>');
    });
    html.push('</ul>');
    html.push('</section>');
  ...
  function renderDetail(id){
    ...
    if(!p){
      app.innerHTML = '<p>Producto no encontrado</p><a class="back-button" href="#/">Volver</a>';
      return;
    }
